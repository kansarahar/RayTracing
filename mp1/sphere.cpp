...
bool Sphere::hit(Ray& r, float tmin, float tmax) const{
  ...
  if (discriminant > 0){
    ...
    if (t2 > tmin && t2 < tmax){
      Hit_Record* sphereHit = new Hit_Record(t2, r.point_at_t(t2), (r.point_at_t(t2)-center).make_unit(), color);
      r.hrl.addHR(*sphereHit);
      return true;
    }
    else{
      if (t1 > tmin && t1 < tmax){
        Hit_Record *sphereHit = new Hit_Record(t1, r.point_at_t(t1), r.point_at_t(t1)-center, color);
        r.hrl.addHR(*sphereHit);
        return true;
      }
    }
